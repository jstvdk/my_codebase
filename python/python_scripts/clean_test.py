...
for run in range(start,stop):
    ...
    with open(f"/fefs/aswg/workspace/vadym.voitsekhovskyi/4LST/output/opt_eff_{start}_{stop}_postpix{post_pix_cut}.txt", 'a') as fout:
        if result:
            fout.write(result)
            fout.write(',')
    with open(f"/fefs/aswg/workspace/vadym.voitsekhovskyi/4LST/output/opt_eff_runStat_{start}_{stop}_postpix{post_pix_cut}.txt", 'a') as fout:
        if result_event:
