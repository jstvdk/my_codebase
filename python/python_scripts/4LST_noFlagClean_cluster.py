...
for run in range(start,stop):
    ...
    with open(f"/fefs/aswg/workspace/vadym.voitsekhovskyi/4LST/opt_eff_noFlagClean_{start}_{stop}.txt", 'a') as fout:
        if result:
            fout.write(result)
            fout.write(',')
    with open(f"/fefs/aswg/workspace/vadym.voitsekhovskyi/4LST/output/opt_eff_noFlagClean_runStat_{start}_{stop}.txt", 'a') as fout:
        if result_event:
