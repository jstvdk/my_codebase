...
for run in range(start,stop):
                ...
                if len(event.muon.tel.keys()):       
                    for tels in event.muon.tel.keys():
                        if not np.isnan(event.muon.tel[tels].efficiency.width):
                            if (event.dl1.tel[tels].parameters.morphology.n_pixels > post_pix_cut and abs(event.muon.tel[tels].parameters.intensity_ratio - 1) < 0.3 and event.muon.tel[tels].parameters.completeness > 0.5 and event.muon.tel[tels].parameters.mean_squared_error.to_value() < 0.01):
    ...
    with open(f"/fefs/aswg/workspace/vadym.voitsekhovskyi/4LST/output/opt_eff_noFlagClean_{start}_{stop}_postpix{post_pix_cut}.txt", 'a') as fout:
        if result:
            fout.write(result)
            fout.write(',')
    with open(f"/fefs/aswg/workspace/vadym.voitsekhovskyi/4LST/output/opt_eff_noFlagClean_runStat_{start}_{stop}_postpix{post_pix_cut}.txt", 'a') as fout:
        if result_event:
            fout.write(result_event)
            fout.write(',')
    with open(f"/fefs/aswg/workspace/vadym.voitsekhovskyi/4LST/output/opt_eff_noFlagClean_allEvents_{start}_{stop}_postpix{post_pix_cut}.txt", 'a') as fout:
