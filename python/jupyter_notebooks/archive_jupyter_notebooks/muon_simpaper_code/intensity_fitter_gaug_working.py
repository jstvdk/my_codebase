...
def create_profile(
    ...
    #print(f"circumeference = {circumference} pixel_diameter = {pixel_diameter} pixels_on_circle {pixels_on_circle}")
...
def create_initial_guess(center_x, center_y, radius, telescope_description):
    ...
    initial_guess["optical_efficiency_muon"] = 0.3
...
class MuonIntensityFitter(TelescopeComponent):
    spe_width = FloatTelescopeParameter(
        help="Width of a single photo electron distribution", default_value=0.1
    ).tag(config=True)
    ...
    max_lambda_m = FloatTelescopeParameter(
        help="Minimum wavelength for Cherenkov light in m", default_value=650e-9
    ).tag(config=True)
    ...
    def __call__(self, tel_id, center_x, center_y, radius, image, pedestal, mask=None):
        ...
        minuit = Minuit(negative_log_likelihood, **initial_guess) #, **step_sizes, **constraints)
        ...
        #impact_error=9999. if not cov else np.sqrt(cov[('impact_parameter', 'impact_parameter')]) 
        #phi_error   =9999. if not cov else np.sqrt(cov[('phi', 'phi')])
        #width_error =9999. if not cov else np.sqrt(cov[('ring_width', 'ring_width')])
        #optical_efficiency_error=9999. if not cov else np.sqrt(cov[('optical_efficiency_muon', 'optical_efficiency_muon')])
        ...
        if (cov is None):
           ...
           cov_valid = (cov.any())
        if (cov_valid):
           cov_valid = (not np.isnan(cov[('impact_parameter', 'impact_parameter')]))

        impact_error = np.sqrt(cov[('impact_parameter', 'impact_parameter')]) if cov_valid else 9999.
        phi_error = np.sqrt(cov[('phi', 'phi')]) if cov_valid else 9999.
        width_error = np.sqrt(cov[('ring_width', 'ring_width')]) if cov_valid else 9999.
        optical_efficiency_error = np.sqrt(cov[('optical_efficiency_muon', 'optical_efficiency_muon')]) if cov_valid else 9999.
        return MuonEfficiencyContainer(
            ...
            impact_error = impact_error * u.m,
            phi_error = phi_error * u.deg,
            width_error = width_error * u.deg,
            optical_efficiency_error = optical_efficiency_error
        )
