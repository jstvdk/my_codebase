...
class SpectrumEvolutionModel:
    def __init__(self, observer):
        ...
        index = -2
        norm = n_primaries * (index + 1) / e0 / ((emax/e0)**((index + 1)) - (emin/e0)**((index + 1)))
        
        self.index = index
        self.norm = norm
        self.e0 = e0
        
    def intrinsic_flux(self, energy):
        return power_law(self.observer.events.primary.energy, self.e0, self.norm, self.index)
    ...
    def primary_sed(self, energy_edges, norm, e0, index, ecut, intrinsic=False):
        ...
        model_flux = power_law(energy, self.e0, self.norm, self.index)
    ...
    def secondary_sed(self, energy_edges, norm, e0, index, ecut, tmin=None, tmax=None):
        ...
        model_flux = power_law(parent_energy, self.e0, self.norm, self.index)
