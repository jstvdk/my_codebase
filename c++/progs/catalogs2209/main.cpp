...
int main(int argc, char *argv[])
{
    ...
    char ch = 'Z';
    char mySpClass[3];
    float myMagnitude = -10.f;
    float myDeltaMag = 100.f;
...
//    cout << cat[0] << "   " << cat[1] << endl;
    ...
    if (argc > 1)
    {
        strcpy(mySpClass, argv[1]);
    }
    else
    {
        memset(mySpClass, 0, 3*sizeof(char));
    }
    if (argc > 2)
    {
        checkmag = true;
        myMagnitude = (float)atof(argv[2]);
        if (argc > 3)
            myDeltaMag = (float)atof(argv[3]);
    }

    cout << myMagnitude << "  " << myDeltaMag << endl;




    ifstream in;    in.open("/Students/astro6/Voitsehovskiy/PROGRAMMING/progs/ppm1.txt");
                    ...
                    while(in)
                    {
                     ...
                     if (mySpClass[0])
                     {
                         if (mySpClass[0] != Buffer.at(24))
                             continue;
                         if (mySpClass[1] && mySpClass[1] != Buffer.at(25))
                             continue;
                     }

                     string formag = Buffer.substr(19,4);
                     //cout << "formag" << formag << endl;
                        double Magnitude = atof(formag.c_str());

                        if(checkmag)
                        {
                            if (!(Magnitude >= myMagnitude && Magnitude <= (myMagnitude + myDeltaMag)))
                            {
                                continue;
                            }
                        }
...
//
//
    ifstream bin;    bin.open("/Students/astro6/Voitsehovskiy/PROGRAMMING/progs/ppm2.txt");
                                                                    i = 0;
                     while(bin)
                          {
                              getline(bin, Buffer);
...
//                          cout << "forbuf - " << i << " = " << forbuf << endl;
    ...
    cout << "after all " << myMagnitude << "  " << myDeltaMag << endl;
...
//    }
    ...
    cout << "determnt x" << dtrmnnt(matrxbaa) << endl;
   cout << "determnt x" << dtrmnnt(matrxaba) << endl;
   cout << "determnt x" << dtrmnnt(matrxaab) << endl;
   cout << "determnt x" << dtrmnnt(matrx) << endl;
...
//        }
        ...
        for (int i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 4; ++j)
            {
                ro = revmatrx[i][j] / sqrt(revmatrx[i][i] * revmatrx[j][j]);
                cout << "\n correlation coefficient for index [" << i << "][" << j << "] = " << ro << endl;
            }
        }

        if (checkmag)
        {
            cout << "\ncalculated for " << myMagnitude << " + " << myDeltaMag << "magnitude" << endl;
        }
